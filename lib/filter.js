...
const getServicesByRegionAndFilter = sortBy => `*[_type == "supportService" && references(^._id) && references(*[_type=="region" && name==$chosenRegion]._id) && references(*[_type=="supportFilterType" && title in $filters]._id)] | order(${sortBy}) { 
  name, 
  "logo": logo.asset->url,
  "filterMatch": filterTypes[]->title match $filters,
  "tags": tags[]->title,
...
const fetchServices = async ({ slug, filters, chosenRegion, sortBy }) => {
  ...
  return {
    supportServices: supportServices.filter(({ filterMatch }) => filterMatch),
    ...rest,
  }
}
